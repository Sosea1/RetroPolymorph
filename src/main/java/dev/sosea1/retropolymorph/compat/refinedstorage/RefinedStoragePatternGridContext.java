...
/**
 * Selection context for a regular RS Pattern Grid.
 *
 * The selected Forge recipe is kept on the shared grid matrix only long enough
 * to drive native preview/encoding. Durable per-player preferences are disabled
 * because NetworkNodeGrid state is shared between viewers.
 */
final class RefinedStoragePatternGridContext implements RecipeSelectionContext {
    ...
    RefinedStoragePatternGridContext(
            Container container,
    ...
    public int getClientStateToken() {
        return this.access.retropolymorph$isRefinedStorageProcessingPattern() ? 1 : 0;
    }

    @Override
    public List<IRecipe> findAllMatches(World world) {
        if (this.access.retropolymorph$isRefinedStorageProcessingPattern()
                || CraftingRules.isRepairCombination(this.matrix)) {
